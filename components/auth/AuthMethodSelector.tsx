'use client';

/**
 * AuthMethodSelector Component
 * 
 * ADHD-friendly auth method selection with:
 * - Clear visual icons
 * - Large tap targets
 * - Satisfying hover/press feedback
 */

import React from 'react';
import { motion } from 'framer-motion';
import { Mail, Phone, Loader2 } from 'lucide-react';
import { cn } from '@/lib/utils';
import { staggerContainer, staggerItem, hoverScale } from '@/lib/motion';
import { AuthMethod, AuthError } from '@/lib/auth/types';

// ============================================
// TYPES
// ============================================

interface AuthMethodSelectorProps {
  onSelect: (method: AuthMethod) => void;
  loading: boolean;
  error: AuthError | null;
  disabled?: boolean;
}

// ============================================
// ICONS
// ============================================

const GoogleIcon = () => (
  <svg viewBox="0 0 24 24" className="w-5 h-5">
    <path
      fill="#4285F4"
      d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
    />
    <path
      fill="#34A853"
      d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
    />
    <path
      fill="#FBBC05"
      d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
    />
    <path
      fill="#EA4335"
      d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
    />
  </svg>
);


// ============================================
// COMPONENT
// ============================================

export function AuthMethodSelector({
  onSelect,
  loading,
  error,
  disabled = false,
}: AuthMethodSelectorProps) {
  const methods = [
    {
      id: 'google' as AuthMethod,
      label: 'Google ile devam et',
      icon: GoogleIcon,
      className: 'bg-white hover:bg-gray-100 text-gray-900 border border-gray-300',
    },
    {
      id: 'phone' as AuthMethod,
      label: 'Telefon ile devam et',
      icon: Phone,
      className: 'bg-[#2a2a2a] hover:bg-[#333] text-white border border-white/20',
    },
    {
      id: 'email' as AuthMethod,
      label: 'E-posta ile devam et',
      icon: Mail,
      className: 'bg-[#2a2a2a] hover:bg-[#333] text-white border border-white/20',
    },
  ];
  
  return (
    <motion.div
      variants={staggerContainer(0.08)}
      initial="hidden"
      animate="visible"
      className="space-y-3"
    >
      {methods.map((method) => {
        const Icon = method.icon;
        const isLoading = loading && method.id === 'google';
        
        return (
          <motion.button
            key={method.id}
            variants={staggerItem}
            whileHover={!disabled && !loading ? { scale: 1.02 } : {}}
            whileTap={!disabled && !loading ? { scale: 0.98 } : {}}
            onClick={() => !disabled && !loading && onSelect(method.id)}
            disabled={disabled || loading}
            className={cn(
              "w-full flex items-center justify-center gap-3 px-4 py-3.5 rounded-xl",
              "font-medium text-sm transition-all duration-200",
              "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2",
              "disabled:opacity-50 disabled:cursor-not-allowed",
              method.className
            )}
          >
            {isLoading ? (
              <Loader2 className="w-5 h-5 animate-spin" />
            ) : (
              <Icon />
            )}
            <span>{method.label}</span>
          </motion.button>
        );
      })}
      
      {/* Divider */}
      <div className="relative my-6">
        <div className="absolute inset-0 flex items-center">
          <div className="w-full border-t border-white/20" />
        </div>
        <div className="relative flex justify-center text-xs uppercase">
          <span className="bg-[#1a1a1a] px-3 text-white/50">
            veya
          </span>
        </div>
      </div>
      
      {/* Info text */}
      <p className="text-xs text-center text-white/60">
        Hesabın varsa otomatik giriş yaparsın,<br />
        yoksa yeni hesap oluştururuz ✨
      </p>
    </motion.div>
  );
}

export default AuthMethodSelector;

