      ...
      {plan.features.map((feature, i) => (
        <li key={i} className="flex items-start gap-3 text-sm">
          <span className="mt-0.5 flex-shrink-0">
            {feature.included ? (
              <Check className={cn("w-4 h-4", feature.premium ? "text-primary" : "text-success")} />
            ) : (
          ...
          <div>
            ...
            {feature.detail && (
              <span className={cn(
                "ml-1 text-xs",
                feature.included ? (feature.premium ? "text-primary/60" : "text-muted-foreground") : "text-muted-foreground"
              )}>
                ({feature.detail})
              </span>
            )}
...
export default function ResultPage() {
  ...
  const interestOptions = [
    { id: 'ai', label: 'AI Dopamin Koçu', icon: '🤖' },
